...
> 两次结果差异均在 ±10% 内且无实质方向性变化（仅 Performance 单项 ↑1，其余持平），判断**没有发生实质性能变化**，属于运行噪声；上述问题均为两次**稳定复现**的存量问题。

## 4. 结论与建议

> ℹ️ 本记录反映的是**目标页（未登录）**真实性能；同日 [[测试记录_2026-08-04_WebPageTest_perfeye]] 因未登录跳转 SSO、测到的是登录页，CLS/TBT/FCP 差异巨大属正常，**两者不可直接对比**。
